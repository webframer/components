...
A toggle input to switch between boolean values (true or false), or a pair of values with two possible outcomes.

Switch input can have any internal value assigned to checked and unchecked states, with optional text labels or custom UI.

It is part of [Universal Input API](/docs/ui/components/Input), with input data `format`, `parse`, `normalize` and event action handlers.
...
<CodeExample desc='Custom Labels'>
