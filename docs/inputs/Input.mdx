...
import { Button, Input, inputControls, Text, View } from '@webframer/ui'
...
> It supports [Universal Input API](/docs/ui/inputs#universal-input-api) and adds extra elements, like help info and error messages, for a consistent UX (user experience).
...
<CodeExample desc='File Input'>
  <Input type='upload' />
</CodeExample>

<CodeExample desc='Multiple Files Uploader'>
  <Input type='file' maxFiles={9} />
</CodeExample>
...
Webframe universal `Input` supports rendering with any custom input control. Add as many controls as you like without worrying about the bundle size because Webframe uses automatic code splitting that lazy loads them on the client side.
