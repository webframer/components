...
import { Button, Icon, Input, Spacer, Text, View } from '@webframer/ui'
...
<AssetCard className='float-right margin' component={Switch} />
...
> Uses [Universal Input API](/docs/ui/inputs#universal-input-api), with input data `format`, `parse`, `normalize` and event action handlers.
...
<CodeExample desc='Reverse Label'>
  <Switch label='Label' reverse />
</CodeExample>

<CodeExample desc='Custom UI Labels'>
  <Switch label={<Text><Icon name='im/switch' /> Switch</Text>}
...
With [Universal Switch Input](/docs/ui/inputs/Input) you can add extra props, like help info and error message.
